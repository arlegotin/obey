...
def test_default_placeholder():
    arg = Argument("some_func", "some_arg", int, PLACEHOLDER_FOR_DEFAUL_VALUE)
    ...
    for arg_type, default, some_other_value in [
        ...
        for with_default in [False, True]:
            for is_option in [False, True]:
                ...
                if with_default:
                    if is_option:
                        arg = Argument(func_name, arg_name, Optional[arg_type], default)
                    else:
                        arg = Argument(func_name, arg_name, arg_type, default)

                    assert arg.value == default
                    arg.validate_value()
                else:
                    if is_option:
                        arg = Argument(func_name, arg_name, Optional[arg_type])
                    else:
                        arg = Argument(func_name, arg_name, arg_type)
    ...
    for t, defaults, values in [
        ...
        for is_option in [False, True]:
            for with_default in [False, True]:
                if with_default:
                    if is_option:
                        arg = Argument(func_name, arg_name, Optional[list[t]], defaults)
                    else:
                        arg = Argument(func_name, arg_name, list[t], defaults)

                    arg.validate_value()
                    assert arg.value == defaults
                else:
                    if is_option:
                        arg = Argument(func_name, arg_name, Optional[list[t]])
                    else:
                        arg = Argument(func_name, arg_name, list[t])
                ...
                for with_default in [False, True]:
                    ...
                    if with_default:
                        ...
                        if is_option:
                            ...
                            arg = Argument(func_name, arg_name, original_tuple)
...
def test_literal():
    ...
    for is_option in [False, True]:
        for with_default in [False, True]:
            if with_default:
                if is_option:
                    arg = Argument(func_name, arg_name, Optional[original_literal], 1)
                else:
                    arg = Argument(func_name, arg_name, original_literal, 1)
            else:
                if is_option:
                    arg = Argument(func_name, arg_name, Optional[original_literal])
                else:
                    arg = Argument(func_name, arg_name, original_literal)
...
def test_nested_literals():
    arg = Argument("some_func", "some_arg", Literal[Literal[1, 2], 3])

    assert arg.original_type == Literal[1, 2, 3]
    assert arg.args_in_type == (1, 2, 3)
    assert arg.is_literal

    arg = Argument("some_func", "some_arg", Literal[1, Literal[Literal[2, 3], 4]])
...
def test_type_description():
    arg = Argument("some_func", "some_arg", int)
    assert arg.type_description == "int"

    arg = Argument("some_func", "some_arg", complex)
    assert arg.type_description == "complex"

    arg = Argument("some_func", "some_arg", Optional[float])
    assert arg.type_description == "float"

    arg = Argument("some_func", "some_arg", Literal["one", "two"])
    assert arg.type_description == "{one|two}"

    arg = Argument("some_func", "some_arg", tuple[int, int, int])
    assert arg.type_description == "[int, int, int]"

    arg = Argument("some_func", "some_arg", tuple[complex, complex])
    assert arg.type_description == "[complex, complex]"

    arg = Argument("some_func", "some_arg", list[float])
    assert arg.type_description == "[float, ...]"


def test_invalid():
    with pytest.raises(Exception):
        Argument("some_func", "some_arg", Callable)

    with pytest.raises(Exception):
        Argument("some_func", "some_arg", tuple[int, float])

    with pytest.raises(Exception):
        Argument("some_func", "some_arg", Literal)

    with pytest.raises(Exception):
        Argument("some_func", "some_arg", Optional)
...
def test_literal_filling():
    arg = Argument("some_func", "some_arg", Literal[2, 3])
